...
abstract contract StandardBridge {
    ...
    function finalizeBridgeERC20(
        ...
        if (_isOptimismMintableERC20(_localToken)) {
            ...
            // L1 OVM_ETH, transfer ETH
            if (_localToken == address(0) && _remoteToken == Predeploys.LEGACY_OVM_ETH) {
                require(_to != address(this), "StandardBridge: cannot send to self");
                require(_to != address(MESSENGER), "StandardBridge: cannot send to messenger");

                bool success = SafeCall.call(_to, gasleft(), _amount, hex"");
                require(success, "StandardBridge: ETH transfer failed");
            } else if (_localToken == address(0) && _remoteToken == Predeploys.LEGACY_ERC20_ETH) {
                // _remoteToken == Predeploys.LEGACY_ERC20_ETH
                // it should set _localToken = METIS (L1)
                deposits[METIS][_remoteToken] = deposits[METIS][_remoteToken] - _amount;
                IERC20(METIS).safeTransfer(_to, _amount);
            } else {
    ...
    function _initiateBridgeERC20(
        ...
        if (_isOptimismMintableERC20(_localToken)) {
            ...
            // L1 _localToken == address(0) _remoteToken == LEGACY_OVM_ETH
            if (_localToken == address(0) && _remoteToken == Predeploys.LEGACY_OVM_ETH) {
                // empty
            } else {
                // it should prevent _localToken != METIS
                if (_remoteToken == Predeploys.LEGACY_ERC20_ETH) {
                    require(
                        _localToken == METIS,
                        "StandardBridge: bridging METIS with fake address"
                    );
                }
