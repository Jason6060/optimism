...
if [ ! -f "$DEVNET/done" ]; then
  # rm docker volume folder for test
...
(
  cd ops-bedrock
  echo "Bringing up L1..."
  if [ $IGNORE_BUILD -eq 1 ]
  then
    echo "Docker image build ignored."
  else
    echo "Docker image build starting..."
  	DOCKER_BUILDKIT=1 docker-compose -f docker-compose-metis.yml build --progress plain
  fi
...
(
  ...
  L2OO_ADDRESS="$L2OO_ADDRESS" \
      docker-compose -f docker-compose-metis.yml up -d op-proposer op-batcher
